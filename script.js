document.addEventListener("DOMContentLoaded", function () {
  ...
  function atualizarResumo() {
    if (!totalProdutosEl || !totalQuantidadeEl || !valorTotalEl) return;

    const totalProdutos = produtos.length;
    const totalQuantidade = produtos.reduce(function (acc, p) {
      return acc + Number(p.quantidade);
    }, 0);

    const valorTotal = produtos.reduce(function (acc, p) {
      return acc + Number(p.quantidade) * Number(p.valor);
    }, 0);
  ...
  function criarLinhaProduto(produto, index) {
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="actions">
          <button class="btn btn-secondary" data-edit="${index}">Editar</button>
          <button class="btn btn-danger" data-delete="${index}">Excluir</button>
        </div>
      </td>
    `;
  ...
  function renderizarTabela() {
    ...
    const filtrados = produtos
      ...
      .filter(function (item) {
        return item.produto.nome.toLowerCase().includes(termo);
      });
  ...
  function adicionarProduto(event) {
    ...
    if (!nome || Number.isNaN(quantidade) || Number.isNaN(valor) || quantidade < 0 || valor < 0) {
  ...
  function editarProduto(index) {
    ...
    if (!novoNome.trim() || Number.isNaN(quantidadeNumero) || Number.isNaN(valorNumero) || quantidadeNumero < 0 || valorNumero < 0) {
      mostrarMensagem(appMessage, "Dados inválidos. Edição cancelada.", "#ef4444");
      return;
    }
  ...
  function exportarCSV() {
    ...
    const linhas = [["nome", "quantidade", "valor"]];
  ...
  function importarCSV(event) {
    ...
    leitor.onload = function (e) {
      try {
        ...
        mostrarMensagem(appMessage, "Erro ao importar CSV.", "#ef4444");
      }
  ...
  if (productTableBody) {
    productTableBody.addEventListener("click", function (event) {
      ...
      if (editBtn) {
        editarProduto(Number(editBtn.getAttribute("data-edit")));
      }

      if (deleteBtn) {
        excluirProduto(Number(deleteBtn.getAttribute("data-delete")));
      }
    });
  }
